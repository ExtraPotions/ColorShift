...
// @version        1.5.2
// @description    Dark charcoal theme for Mana Pool — collapsible home sections + collapse-all by avatar
// @author         ExtraPotions
// @match          *://manapool.com/*
...
(function () {
  ...
  function setAllCollapsed(collapsed) {
    ...
    // If sections not enhanced yet, still persist intended state for titles
    var map = loadCollapsed();
  ...
  function ensureCollapseAllButtons() {
    var triggers = document.querySelectorAll('button[aria-label="Open profile menu"]');
    if (!triggers.length) return;

    // Remove leftovers when leaving home
    if (!isHome()) {
      document.querySelectorAll('.mpge-collapse-all').forEach(function (b) { b.remove(); });
      return;
    }

    triggers.forEach(function (avatarBtn) {
      var parent = avatarBtn.parentElement;
      if (!parent) return;
      if (parent.querySelector(':scope > .mpge-collapse-all')) return;

      var btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'mpge-collapse-all';
      ...
      parent.insertBefore(btn, avatarBtn);
    });
    syncCollapseAllButtons();
  }
