...
const brandIcon=`data:image/png;base64,`+fs.readFileSync(path.join(root,'assets/theme-picker-icon.png')).toString('base64');
...
for(const [site,name]of sites){
  const metadata=[['name',name+' Theme Picker'],['namespace',repo],['version',version],['description','Theme palettes, accessible settings and site enhancements.'],['author','ExtraPotions'],['license','CC-BY-NC-4.0'],['icon',`${raw}/assets/theme-picker-icon.png`],['match',`*://${site}.com/*`],['match',`*://www.${site}.com/*`],['run-at','document-start'],['downloadURL',`${repo}/releases/latest/download/${site}-theme-picker.user.js`],['updateURL',`${repo}/releases/latest/download/${site}-theme-picker.user.js`],['require',`${raw}/theme-picker-common.js`],['grant','GM_getValue'],['grant','GM_setValue'],['grant','GM_registerMenuCommand']];
  ...
  fs.writeFileSync(path.join(root,site+'-theme-picker.user.js'),'// ==UserScript==\n'+metadata.map(([k,v])=>'// @'+k.padEnd(15)+v).join('\n')+'\n// ==/UserScript==\n'+warning+adapter.replace('__SITE__',site).replace('__ICONS__',JSON.stringify({[site]:brandIcon}))+'\n}\n');
}
console.log('Built '+(sites.length+1)+' synchronized release files for '+version);
