...
(function () {
  ...
  function ensureStyle() {
    ...
    if (GE.isThemeEnabled && !GE.isThemeEnabled()) {
      ...
      node.textContent = '/* Scryfall Grey Edition — original (site theme) */';
      clearInlineTheme();
      return;
    }
  ...
  function darkenLightSurfaces() {
    ...
    for (var j = 0; j < list.length; j++) {
      ...
      if (el.id === 'grey-edition-fab' || el.id === 'grey-edition-fab-panel') continue;
      if (el.closest && el.closest('#grey-edition-fab, #grey-edition-fab-panel, .card-image, picture, svg, img, .card-grid-item-card, .card-face')) continue;
