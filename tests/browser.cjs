...
(async()=>{
  ...
  try {
    for(const site of ['manapool','scryfall','steamgifts','tcgplayer']) {
      ...
      assert(siteCode.includes(`// @icon           https://raw.githubusercontent.com/ExtraPotions/super-octo-parakeet/theme-picker-${version}/assets/theme-picker-icon.png`));
      ...
      const fabIcon=await fab.locator('img').evaluate(el=>({src:el.getAttribute('src'),width:el.getBoundingClientRect().width,height:el.getBoundingClientRect().height}));
      assert.match(fabIcon.src,/^data:image\/png;base64,/);
      assert.equal(Math.round(fabIcon.width),48);
      assert.equal(Math.round(fabIcon.height),48);
