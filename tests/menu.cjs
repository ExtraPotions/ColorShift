...
await panel.locator(':scope > .settings-group > summary').first().click();
...
for(const [palette,background] of Object.entries({lightGray:'rgb(63, 63, 60)',darkGray:'rgb(37, 37, 34)',navy:'rgb(26, 35, 50)',black:'rgb(10, 10, 10)',fireRed:'rgb(33, 21, 22)',leafGreen:'rgb(19, 29, 23)',heartGold:'rgb(33, 29, 19)'})){
 await theme.selectOption(palette);assert.equal(await panel.evaluate(el=>getComputedStyle(el).backgroundColor),background,palette+' menu follows theme');
}
await theme.selectOption('original');await page.emulateMedia({colorScheme:'light'});await page.waitForFunction(()=>getComputedStyle(document.getElementById('colorshift-root').shadowRoot.querySelector('.panel')).backgroundColor==='rgb(244, 244, 242)');
