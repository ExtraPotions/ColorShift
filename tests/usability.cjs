...
await context.addInitScript({content:fs.readFileSync(require("./edition-path.cjs")('colorshift-manapool.user.js'),'utf8')});const page=await context.newPage();await page.goto('https://manapool.com/');await page.locator('#colorshift-fab').click();await page.evaluate(()=>document.getElementById('colorshift-root')?.shadowRoot.querySelectorAll('details').forEach(el=>el.open=true));const panel=page.getByRole('dialog'),theme=panel.getByRole('combobox',{includeHidden:true,name:'Theme',exact:true});
...
await panel.getByRole('button',{name:'Reset appearance',exact:true}).click();assert.equal(await theme.inputValue(),'system');assert.equal(await panel.getByRole('switch',{name:'Brighter links',exact:true}).getAttribute('aria-checked'),'true');
...
page.once('dialog',d=>d.accept());await panel.getByRole('button',{name:'Reset defaults',exact:true}).click();assert.equal(await theme.inputValue(),'system');
await page.reload();await page.locator('#colorshift-fab').click();await page.evaluate(()=>document.getElementById('colorshift-root')?.shadowRoot.querySelectorAll('details').forEach(el=>el.open=true));assert.equal(await theme.inputValue(),'system');
