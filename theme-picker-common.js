 ...
 *
 * Shared palette + settings for ManaPool / Scryfall / SteamGifts Theme Picker.
...
(function (global) {
  ...
  function registerMenus(site) {
    ...
    GM_registerMenuCommand(
      'Theme Picker: Palette → ' + (PALETTES[next] && PALETTES[next].label || next),
      function () {
    ...
    GM_registerMenuCommand(
      get('brighterLinks', false) ? 'Theme Picker: Brighter links OFF' : 'Theme Picker: Brighter links ON',
      function () {
    ...
    GM_registerMenuCommand(
      get('hideAds', false) ? 'Theme Picker: Hide ads OFF' : 'Theme Picker: Hide ads ON',
      function () {
    ...
    if (site === 'manapool') {
      GM_registerMenuCommand(
        get('dense', false) ? 'Theme Picker: Denser grid OFF' : 'Theme Picker: Denser grid ON',
        function () {
  ...
  function paletteCss() {
    ...
    if (p === 'original') {
      return '/* Theme Picker palette: original (site default — theme CSS suppressed by host script) */';
    }
    var t = PALETTES[p];
    // darkGray matches each script's baked-in charcoal — no remaps needed
    if (p === 'darkGray') {
      return [
        '/* Theme Picker palette: darkGray (default) */',
  ...
  function setSetting(site, key, value) {
    ...
    try {
      var flagStyle = document.getElementById('theme-picker-steamgifts-flags') ||
        document.getElementById('theme-picker-common-flags');
      if (!flagStyle) {
        flagStyle = document.createElement('style');
        flagStyle.id = 'theme-picker-common-flags';
  ...
  function buildPanel(site) {
    ...
    panel.setAttribute('aria-label', 'Theme Picker settings');
      ...
      try {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
          navigator.clipboard.writeText(json).then(function () {
            alert('Theme Picker settings copied to clipboard.');
          }, function () {
    ...
    tools.appendChild(mkTool('Import', function () {
      var raw = window.prompt('Paste Theme Picker settings JSON:');
      if (!raw) return;
      try {
    ...
    tools.appendChild(mkTool('Reset', function () {
      if (!window.confirm('Reset Theme Picker settings to defaults?')) return;
      Object.keys(SETTINGS_DEFAULTS).forEach(function (k) {
  ...
  function mountSettingsFab(site, iconUrl) {
    ...
    function mount() {
      ...
      btn.id = FAB_ID;
      btn.type = 'button';
      btn.title = 'Theme Picker settings';
      btn.setAttribute('aria-label', 'Theme Picker settings');
